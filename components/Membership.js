  ...
  const handlePaymentWithSwish = async (values) => {
    ...
    try {
      ...
      if (paymentRequest && paymentStatus === 'pending') {
        setError('En betalning pågår redan. Vänligen vänta eller ladda om sidan för att försöka igen.');
        setIsLoading(false);
        return;
      }
          ...
          try {
            ...
            if (["DECLINED", "ERROR", "CANCELLED"].includes(payment.status)) {
              ...
              try {
                ...
                setError('Betalningen lyckades men ett fel uppstod när medlemskapet skulle skapas. Kontakta support.');
                setPaymentStatus('error');
              }
              setIsLoading(false);

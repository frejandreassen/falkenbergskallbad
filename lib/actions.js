...
export async function getHeader() {
  ...
  export async function bookSlot(slot, selectedSeats){
    //Check that slot is still available
    const res = await fetch(`https://cms.falkenbergskallbad.se/items/slots/${slot.id}`)
    const currentSlot = res.data
    if (currentSlot.available_seats < selectedSeats) throw new Error('Not enough seats available')

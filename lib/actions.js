      ...
      `${process.env.NEXT_PUBLIC_DIRECTUS_URL}/items/medlemmar`,
      {
        ...
        body: JSON.stringify({
          ...
          betalningar: [
            {
              datum: paymentDate,
              belopp: payment.amount || 200, // Fallback to default amount if not provided
...
async function createBooking(order, user, slot, transactionId) {
  try {
    const access_code = await createCode(user.email, slot);
    const bookingData = {
      user: user.id,
      slot: order.slotId,
      booked_seats: order.selectedSeats,
      transaction: transactionId,
      door_code: access_code.code,
...
export async function bookUsingSwish(order, paymentRequestId) {
  try {
    // First check if a booking already exists for this transaction
    ...
    // Proceed with creating new booking
    const res = await fetch(
      `https://cms.falkenbergskallbad.se/items/transactions/${paymentRequestId}?${accessTokenParam}`,
    );
    const data = await res.json();
...
export async function cancelBooking(id) {
  try {
    ...
    if (isBefore(cancellationDeadline, startTime)) {
      // Delete the access code from Seam if it exists
      if (bookingDetails.data.access_code_id) {
        try {
          const seamResponse = await fetch(
            "https://connect.getseam.com/access_codes/delete",
            {
              ...
              body: JSON.stringify({
                access_code_id: bookingDetails.data.access_code_id
              }),
            }
          );
          
          if (!seamResponse.ok) {
            console.error("Failed to delete Seam access code:", await seamResponse.text());
            // Continue with cancellation even if Seam deletion fails
          }
        } catch (seamError) {
          console.error("Error deleting Seam access code:", seamError);
          // Continue with cancellation even if Seam deletion fails
        }
      }

...
export async function getSlots() {
  const res = await fetch(
    'https://cms.falkenbergskallbad.se/items/slots?filter={"start_time": {"_gte": "$NOW(-0.5 hours)"}}&limit=-1&access_token=' +
...
async function createCode(email) {
  ...
  const access_code = String(numericHash % 10000).padStart(4, '0');
  return access_code;
}
